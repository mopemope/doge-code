...
mod integration_tests {
    ...
    fn create_test_repomap() -> RepoMap {
        let symbols = vec![
            // Large file with large function
            SymbolInfo {
                name: "large_function".to_string(),
                kind: SymbolKind::Function,
                file: PathBuf::from("src/large_file.rs"),
                start_line: 1,
                start_col: 1,
                end_line: 150,
                end_col: 1,
                parent: None,
                file_total_lines: 600,
                function_lines: Some(150),
            },
            // Large file with small function
            SymbolInfo {
                ...
                parent: None,
                file_total_lines: 600,
                function_lines: Some(20),
            },
            // Small file with medium function
            SymbolInfo {
                name: "medium_function".to_string(),
                kind: SymbolKind::Function,
                file: PathBuf::from("src/small_file.rs"),
                start_line: 1,
                start_col: 1,
                end_line: 80,
                end_col: 1,
                ...
                function_lines: Some(80),
            },
            // Struct in large file
            SymbolInfo {
    ...
    fn test_find_large_files() {
        let repomap = create_test_repomap();
        ...
        let results = tools.search_repomap(&repomap, args).unwrap();

        assert_eq!(results.len(), 1);
        assert_eq!(results[0].file, PathBuf::from("src/large_file.rs"));
        assert_eq!(results[0].file_total_lines, 600);
        assert_eq!(results[0].symbol_count, 3); // large_function, small_function, LargeStruct
    }
    ...
    fn test_find_large_functions() {
        ...
        // Should find files containing functions with >= 100 lines
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].file, PathBuf::from("src/large_file.rs"));
        assert_eq!(results[0].symbols.len(), 1); // Only large_function matches
        assert_eq!(results[0].symbols[0].name, "large_function");
    }
    ...
    fn test_combined_filters() {
        ...
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].symbols.len(), 1); // Only large_function matches all criteria
        assert_eq!(results[0].symbols[0].name, "large_function");
    }

    #[test]
    fn test_sort_by_file_lines_desc() {
        let repomap = create_test_repomap();
        let tools = RepomapSearchTools::new();
        ...
        assert_eq!(results.len(), 2);
        assert_eq!(results[0].file_total_lines, 600); // large_file.rs first
        assert_eq!(results[1].file_total_lines, 100); // small_file.rs second
    }
}
