...
impl FsTools {
    ...
    pub fn fs_list(&self, path: &str, max_depth: Option<usize>, pattern: Option<&str>) -> Result<Vec<String>> {
        list::fs_list(&self.root, path, max_depth, pattern)
    }

    pub fn fs_read(&self, path: &str, offset: Option<usize>, limit: Option<usize>) -> Result<String> {
        read::fs_read(&self.root, path, offset, limit)
    }

    pub fn fs_search(&self, pattern: &str, include: Option<&str>) -> Result<Vec<(PathBuf, usize, String)>> {
