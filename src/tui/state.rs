...
impl LogEntry {
    pub fn render(&self, width: usize, theme: &Theme) -> Vec<crate::tui::style_utils::StyledLine> {
        match self {
            LogEntry::Plain(text) => {
                crate::tui::style_utils::render_plain_entry(text, width, theme)
            }
            LogEntry::Markdown(text) => {
                crate::tui::style_utils::render_markdown_entry(text, width, theme)
            }
        }
    }
}
...
pub struct RenderPlan {
    ...
    pub log_lines: Vec<crate::tui::style_utils::StyledLine>,
