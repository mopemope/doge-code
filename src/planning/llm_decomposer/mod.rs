...
// Keep public API surface similar to previous file by re-exporting core types and impl
pub use self::inner::*;

mod inner {
    // Inline the previous content of llm_decomposer.rs but with module-local path adjustments.
    ...
    impl LlmTaskDecomposer {
        ...
        async fn gather_project_context(&self) -> Result<ProjectContext> {
            ...
            if let Some(repomap) = self.repomap.read().await.as_ref() {
                // Identify key files (unique file paths)
            ...
            for line in lines {
                    ...
                    || line.starts_with("2.")
                {
                    ...
                    if !description.is_empty() && description.len() > 10 {
                        let step_type =
                            crate::planning::llm_decomposer::infer::infer_step_type(&description);
